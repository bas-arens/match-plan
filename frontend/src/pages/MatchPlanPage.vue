    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">
          ...
          <span v-if="planStore.result?.elapsed_ms != null" class="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400">
            <Timer :size="12" />
            {{ planStore.result.elapsed_ms >= 1000 ? (planStore.result.elapsed_ms / 1000).toFixed(1) + 's' : planStore.result.elapsed_ms + 'ms' }}
          </span>
        </p>
      </div>
